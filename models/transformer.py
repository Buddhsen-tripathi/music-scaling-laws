...
Based on GPT-2 architecture, simplified for scaling experiments.
"""
...
class TransformerConfig:
    ...
    def num_params(self) -> int:
        """Estimate number of parameters (excluding embeddings for fair comparison)."""
        # Embedding: vocab_size * n_embd + block_size * n_embd
        emb = self.vocab_size * self.n_embd + self.block_size * self.n_embd
        # Per layer: attention (4 * n_embd^2) + MLP (8 * n_embd^2) + layernorms
        per_layer = 4 * self.n_embd**2 + 8 * self.n_embd**2 + 4 * self.n_embd
        layers = self.n_layer * per_layer
        # Final layernorm + lm_head
        final = self.n_embd + self.vocab_size * self.n_embd
...
TRANSFORMER_CONFIGS = {
    "tiny": TransformerConfig(n_layer=4, n_head=4, n_embd=128),      # ~1M
